...
4. Start from [assets/blank-paper-explainer.html](assets/blank-paper-explainer.html). Preserve its layout, notes rail, citation styling, MathJax fallback, and accessible lightbox. Put reusable term/formula explanations in `notes-data.sections`; put full bibliography entries once in `notes-data.bibliography`. Read [references/guide.md](references/guide.md) only when changing the content or implementation contract.
...
- Keep citation markers such as `[1]` in prose. Each marker must resolve through the global `notes-data.bibliography` map and be listed once in its section's `citations` number array.
- Keep right-rail tabs exactly `專有名詞`, `公式涵義`, `引用`; empty arrays are valid. Keep formula fallbacks readable without MathJax.
