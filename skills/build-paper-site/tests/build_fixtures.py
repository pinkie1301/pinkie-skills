...
def build_section(section: str, title: str, manifest: dict[str, object], position: int) -> str:
    ...
    return f'<section id="{section}" data-title="{chapter_label}"><div class="chapter-label" data-chapter-label>{chapter_label}</div><h2>{title}</h2><p data-claim-id="{claim_id}" data-evidence-ids="{evidence_id}">{title}以完整段落說明背景、方法或結果；專有名詞與公式涵義由同章右欄補充。{badge(evidence_id, kind, locator)}</p>{artifact}</section>'
